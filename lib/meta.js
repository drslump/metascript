...
function Meta() {
  ...
  meta.codegenObjectProperty = function (key, value, kind) {
    ...
    if (/^\w+$/.test(key)) {
      key = meta.codegenIdentifier(value.loc, key);
    } else {
  ...
  meta.newDestructuringCodegenContext = function (expr) {
    var context = new meta.CodegenContext(expr);
    context.destructuring = true;
    return context;
  };
  ...
  meta.objectSymbol = new meta.Symbol('<object>', 'builtin',
    new meta.SymbolData(meta.aritySequence, 'NONE', {
      ...
      codegenAsExpression: function (expr, context) {
        ...
        var result = meta.codegenObject(expr.loc, members);
        if (context.destructuring) {
  ...
  meta.arraySymbol = new meta.Symbol('<array>', 'builtin',
    new meta.SymbolData(meta.aritySequence, 'NONE', {
      ...
      codegenAsExpression: function (expr, context) {
        ...
        var result = meta.codegenArray(expr.loc, members);
        if (context.destructuring) {
  ...
  meta.codegenAssignmentAsExpression = function (expr, context) {
    ...
    if (leftArity === 1) {
      ...
      var leftContext = meta.newDestructuringCodegenContext(expr, context);
      var l = left.codegenAsExpression(leftContext);
  ...
  meta.codegenAssignmentToTemporary = function (expr, context, tmp) {
    ...
    if (tmp !== null) {
      ...
      if (leftArity === 1) {
        ...
        var leftContext = meta.newDestructuringCodegenContext()
        var argL = left.codegenAsExpression(leftContext);
  ...
  meta.KEY_ROOT_SCOPE.set('var', new meta.Symbol('var', 'builtin',
    new meta.SymbolData(meta.arityUnary, 'VAR', {
      ...
      fixTagKind: function (expr) {
        ...
        function handleDestructuring(arg) {
          if (arg.isTuple() || arg.isObject() || arg.isArray()) {
            arg.forEach(handleDestructuring);
          } else if (arg.isProperty()) {
            handleDestructuring(arg.at(1));
          } else {
            // console.log(arg.sym.id, arg.printAst())
            arg.handleAsTagDeclaration();
          }
        }
        handleDestructuring(expr.at(0));

        expr.transformInto(expr.at(0));
      },
      resolve: function (expr, vTagMap) {
        if (vTagMap !== null) {
    ...
    new meta.SymbolData(meta.arityBinary, 'FUNCTION', {
      ...
      codegenAsExpression: function (expr) {
        ...
        if (args.sym === meta.tupleSymbol) {
          for (var i = 0; i < args.argCount(); i++) {
            var arg = args.argAt(i);
            if (!arg.isFunctionArgument() && !arg.isObject()) {
            ...
            if (arg.isTag()) { 
              ...
              // ÜBERHACK! Force arity for params otherwise codegen fails
              arg.resultCount = 1;
              arg.forEachRecursive(function (it) {
                if (it.isFunctionArgument())
                  it.resultCount = 1;
              })

              var context = meta.newDestructuringCodegenContext();
              parameters.push(arg.codegenAsExpression(context));
            }
          }
        } else {
      ...
      fixTagKind: function (expr) {
        ...
        function handleDestructuring (arg) {
          if (arg.isTuple() || arg.isArray() || arg.isObject()) {
            arg.forEach(handleDestructuring);
          } else if (arg.isProperty()) {
            handleDestructuring(arg.at(1));
          } else {
            arg.handleAsFunctionArgument();
          }
        }
        handleDestructuring(expr.at(0));
      },
  ...
  Compiler.prototype.generate = function (jsAst, escodegenOptions) {
    ...
    if (this.options.es5Destructuring) {
      this.es5Destructuring(jsAst);
    }
  ...
  Compiler.prototype.es5Destructuring = function (jsAst) {
      var constructuring = require('constructuring');
      constructuring.transform(jsAst)
  };
