  ...
  meta.KEY_ROOT_SCOPE.set('var', new meta.Symbol('var', 'builtin',
    new meta.SymbolData(meta.arityUnary, 'VAR', {
      ...
      fixTagKind: function (expr) {
        ...
        expr.forEachRecursive(function (it) {
          // Ignore property names
          if (it.parent.isProperty() && it.parent.indexOf(it) === 0) {
            return;
          }
          if (it.isTag() && !it.isTagDeclaration()) {
            it.handleAsTagDeclaration();
          }
        });
  ...
  meta.functionSymbol = new meta.Symbol('->', 'builtin',
    new meta.SymbolData(meta.arityBinary, 'FUNCTION', {
      ...
      fixTagKind: function (expr) {
        ...
        expr.args[0].forEachRecursive(function (it) {
          // Ignore property names
          if (it.parent.isProperty() && it.parent.indexOf(it) === 0) {
            return;
          }
          if (it.isTag() && !it.isFunctionArgument()) {
            it.handleAsFunctionArgument();
          }
        });
