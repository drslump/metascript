...
#keepmacro :=
'''
  Shortcut for variable declaration and initialization

    foo := 'foo'  ;; var foo = 'foo'

  When assigning to anything other than a tag the declaration
  is simply ignored and just the assignment takes place
...
'''
  ...
  expand: (lhs, rhs) ->
    ; We can't use `var` on quoted text so we need to declare it manually
    if (lhs.tag?()) do
      lhs.handle-as-tag-declaration()
    ; Assignment has lower precedence than quotation so we need to use parens
    ` ( (~`lhs) = (~`rhs) )
